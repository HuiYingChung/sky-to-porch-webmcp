...
function evaluateHeatEvidence(result: HeatQueryResult): EvidenceEvaluationResult {
  const evidence = result.evidence;
  if (!evidence) throw new Error("Heat result has no evidence to evaluate");
...
export async function finalizeHeatQueryResult(
  ...
  const evaluation = evaluateHeatEvidence(adapterResult);
  const assessments = separateHeatEvidence(evaluation.evidence);
