...
import { parseCoverageGapInput, queryAirQualityEvidence } from "@/lib/coverage-gap/service";
...
export async function POST(request: Request) {
  try {
    ...
    // ADR-0045: air joins the guarded Granite chain. Coverage-gap queries are
    ...
    const adapterResult = await queryAirQualityEvidence(input, fetchImpl ? { fetchImpl } : {});
    ...
    console.error("[air-query] unhandled route error", error);
