  ...
  return (
    <div role="status" data-testid="drought-rejection-panel">
      ...
      <p style={{ margin: "8px 0 0", color: "var(--status-warning-fg)", fontSize: "14px" }}>
        Missing regional data is not proof of normal conditions. No Tucson/Arizona fixture was substituted.
      </p>
  ...
  return (
    <div data-testid="drought-evidence-panel" style={{ fontSize: "14px" }}>
      ...
      <p style={{ marginTop: 0 }}>
        ...
        {dataModeLabel(evidence.dataMode)} · <strong>Assembled:</strong> {evidence.assembledAt}
      </p>
        ...
        <div style={{ display: "grid", gap: "8px" }}>
          {evidence.observations.map((observation) => (
            <article key={observation.observationId} data-testid="drought-observation"
              ...
              />
              <strong>{observation.variableName}</strong>
              <div>Dataset/product: {observation.provenance.product}</div>
              <div>Source: {observation.provenance.sourceId}</div>
              <div>Observed: {observation.provenance.observedAt}</div>
              <div>Retrieved: {observation.provenance.retrievedAt}</div>
              <div>Value: {observation.value !== undefined
                ? `${observation.value} ${observation.unit ?? ""}`
                : observation.textValue ?? "Visualization only; no numeric value inferred"}</div>
